            ...
            if (input.IsNewKeyPress(Keys.Space) || input.IsNewKeyPress(Keys.Enter) || input.IsNewButtonPress(Buttons.A))
            {
                if (!keyHit)
                {
                    ...
                    if(levelNumber <= Level.numberOfLevels)
                        LevelLoadingScreen.Load(ScreenManager, true, PlayerIndex.One, levelNumber, new GameplayScreen(levelNumber));
                    else
                        LoadingScreen.Load(ScreenManager, false, null, new BackgroundScreen(), new CreditsScreen());

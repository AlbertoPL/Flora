...
namespace GameStateManagement
{
    ...
    class FireScreen : MiniGameScreen
    {
        ...
        public void LoadContent(ContentManager content, ScreenManager screenManager)
        {
            ...
            background = content.Load<Texture2D>("Background/FireScreenBackGroundWithLine");
            ...
            aboutToLoseSound = content.Load<SoundEffect>("Sound/Almost_Lose");
                ...
                for (int j = 0; j < fireGrid.GetLength(1); j++)
                {
                    if (IsValidFireObjectPosition(i, j) && fireGrid[i, j] != null)
                    {
                        ...
                        if (j == fireGrid.GetLength(1) - 1) //If fire reaches the 'bottom'
                            OnLose();
                        if (j == fireGrid.GetLength(1) - 2) //If fire almost reaches the 'bottom'
                            fireOnLastRow = true;
                    }
                }
            }
            if (fireOnLastRow && !isAboutToLose)
            {
                isAboutToLose = true;
                aboutToLoseSound.Play();
            }
            else if (!fireOnLastRow && isAboutToLose)
            {
                isAboutToLose = false;
            }
        }
